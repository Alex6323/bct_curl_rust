...
pub fn troika(out_buf: &mut [Trit], in_buf: &[Trit]) {
    troika_var_rounds(out_buf, in_buf, NUM_MAX_ROUNDS);
}
...
fn sub_trytes(state: &mut [Trit]) {
    for sbox_idx in 0..NUM_SBOXES {
        let sbox_input = 9 * state[3 * sbox_idx]
            + 3 * state[3 * sbox_idx + 1]
            + state[3 * sbox_idx + 2];
...
fn troika_absorb(
    ...
    while message_length >= rate {
        ...
        for trit_idx in 0..rate {
            //state[trit_idx] = TROIKA_TROIKAFY!(message[message_offset + trit_idx],
            // Trit);
