...
fn main() {
    println!("+-------------------------------------------------------------------------+");
    println!("| This tool compares several different IOTA hashing functions, namely     |");
    println!("| various Curl and Troika implementations.                                |");
    println!("| As a baseline serves xxHash, a very fast commonly used hashing function.|");
    println!("|                                                                         |");
    println!("| Disclaimer: These benchmarks do not satisfy scientific standards and    |");
    println!("| are just intended to get a rough idea about the performance of each     |");
...
fn bench_functional_curl(num_hashes: usize) {
    ...
    let start = Instant::now();
    for i in 0..transactions_as_trits.len() {
        let hash_trits = fcurl(&transactions_as_trits[i], NUM_CURL_ROUNDS);
    }
    ...
    pool.scoped(|scope| {
        for _ in 0..num_threads {
            scope.execute(|| {
                ...
                for j in offset..offset + chunk_length {
                    let hash_trits = fcurl(&transactions_as_trits[j], NUM_CURL_ROUNDS);
                }
            })
        }
    });
...
fn bench_functional_bct_curl_64(num_hashes: usize) {
    ...
    let start = Instant::now();
    let hash_trits = bct_fcurl_64(&transactions_as_trits, NUM_CURL_ROUNDS);
...
fn bench_functional_bct_curl_128(num_hashes: usize) {
    ...
    let start = Instant::now();
    let hash_trits = bct_fcurl_128(&transactions_as_trits, NUM_CURL_ROUNDS);
...
fn bench_functional_bct_curl_64_par(num_hashes: usize) {
    print_title(&format!(
        "Functional BCT-Curl/64 ({} threads)",
        num_cpus::get()
    ));
    ...
    let start = Instant::now();
    let hash_trits = bct_fcurl_64_par(&transactions_as_trits, NUM_CURL_ROUNDS);
...
fn bench_simd_functional_bct_curl_64(num_hashes: usize) {
    ...
    let start = Instant::now();
    let hash_trits = simd_bct_fcurl_64(&transactions_as_trits, NUM_CURL_ROUNDS);
...
fn bench_simd_functional_bct_curl_64_par(num_hashes: usize) {
    print_title(&format!(
        "SIMD Functional BCT-Curl/64 ({} threads)",
        num_cpus::get()
    ));
    ...
    let start = Instant::now();
    let hash_trits = simd_bct_fcurl_64_par(&transactions_as_trits, NUM_CURL_ROUNDS);
